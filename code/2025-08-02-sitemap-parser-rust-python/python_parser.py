...
class PythonSitemapParser:
    ...
    def __init__(self, max_concurrent: int = 10, timeout: int = 30, 
                 max_sitemaps: int = 10, max_depth: int = 2, max_nested_per_level: int = 5):
        self.max_concurrent = max_concurrent
        self.timeout = timeout
        self.max_sitemaps = max_sitemaps  # How many top-level sitemaps to process
        self.max_depth = max_depth  # How deep to recurse into nested sitemaps
        self.max_nested_per_level = max_nested_per_level  # How many nested sitemaps per level
        self.session = None
    ...
    async def _process_single_sitemap(self, sitemap_url: str, base_url: str, max_depth: int = 1) -> tuple[Set[str], int]:
        """Process a single sitemap with recursive processing of nested sitemaps."""
        logger.debug(f"Processing single sitemap: {sitemap_url} (depth: {max_depth})")
        ...
        content = await self._fetch_url(sitemap_url)
        if not content:
            raise Exception(f"Could not fetch sitemap: {sitemap_url}")

        urls, nested_sitemaps = self._parse_sitemap_xml(content, base_url)
        request_count = 1

        # Process nested sitemaps if we have depth remaining
        if nested_sitemaps and max_depth > 1:
            logger.debug(f"Processing {len(nested_sitemaps)} nested sitemaps at depth {max_depth-1}")
            
            # Limit nested processing to avoid explosion (configurable limit)
            limited_nested = nested_sitemaps[:self.max_nested_per_level]
            
            nested_tasks = [
                self._process_single_sitemap(nested_url, base_url, max_depth - 1)
                for nested_url in limited_nested
            ]
            ...
            for nested_result in nested_results:
                if isinstance(nested_result, Exception):
                    logger.error(f"Error processing nested sitemap: {nested_result}")
                else:
                    nested_urls, nested_requests = nested_result
                    urls.update(nested_urls)
                    request_count += nested_requests
        
        logger.debug(f"Found {len(urls)} total URLs and {len(nested_sitemaps)} nested sitemaps in {sitemap_url}")
    ...
    async def parse_site(self, base_url: str) -> SitemapResult:
        ...
        try:
            ...
            if robots_content:
                ...
                # Process configurable number of sitemaps with configurable depth
                limited_sitemaps = result.sitemaps_found[:self.max_sitemaps]
                logger.info(f"Processing first {len(limited_sitemaps)} sitemaps out of {len(result.sitemaps_found)} total (max_sitemaps={self.max_sitemaps})")
                
                # Process sitemaps concurrently for better performance comparison
                tasks = [
                    self._process_single_sitemap(sitemap_url, normalized_url, max_depth=self.max_depth)
                    for sitemap_url in limited_sitemaps
                ]
                
                sitemap_results = await asyncio.gather(*tasks, return_exceptions=True)
                
                for i, sitemap_result in enumerate(sitemap_results):
                    if isinstance(sitemap_result, Exception):
                        error_msg = f"Error processing {limited_sitemaps[i]}: {sitemap_result}"
                        result.errors.append(error_msg)
                        logger.error(error_msg)
                    else:
                        urls, requests = sitemap_result
...
def parse_sitemaps_sync(base_urls: List[str], max_concurrent: int = 10, 
                       max_sitemaps: int = 10, max_depth: int = 2, max_nested_per_level: int = 5) -> List[SitemapResult]:
    """Synchronous wrapper for parsing sitemaps.
    
    Args:
        base_urls: List of base URLs to parse
        max_concurrent: Maximum concurrent connections
        max_sitemaps: Maximum number of top-level sitemaps to process
        max_depth: Maximum recursive depth for nested sitemaps
        max_nested_per_level: Maximum nested sitemaps to process per level
        
    Returns:
        List of SitemapResult objects
    """
    async def run():
        async with PythonSitemapParser(
            max_concurrent=max_concurrent, 
            max_sitemaps=max_sitemaps,
            max_depth=max_depth,
            max_nested_per_level=max_nested_per_level
        ) as parser:
